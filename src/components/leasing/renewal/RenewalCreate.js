...
class RenewalCreate extends React.Component {
  _isMounted = false;
  dismissAlert = () => {
    this.setState({
      visible: false,
  ...
  selectOffer = (offerDtlId) => {
    ...
    for (var i = 0; i < value.length; i++) {
      if (value[i]["offerDtlId"] === offerDtlId) {
        ...
        this.state.leaseEnd = value[index]["leaseEndDate"];
  ...
  async componentDidMount() {
    ...
    await axios
      .post("/renewal/offers/view/", {
        leaseOfferId: paramValue1,
      })
      .then((response) => {
        this.state.offerListData = response.data.offerDtl;
        this.state.offerListData.forEach((rData) => {
          nData.leaseEndDate = moment(rData.leaseEndDate).format("MM-DD-YYYY");
          nData.leaseType = rData.leaseType;
          nData.rent = rData.rent;
          nData.offerDtlId = rData.offerDtlId;
          rowDataList.push(nData);
          this.setState((prevState) => ({
            rowOfferDtl: [...prevState.rowOfferDtl],
          }));
          this.setState({
            rowOfferDtl: rowDataList,
            tenantId: response.data.offerHdr.appReqId,
          });
        });
      })
      .catch((error) => {
    ...
    await axios
      .post("/lease/applicant-search-byid/", {
        appReqstId: this.state.tenantId,
      })
      .then((response) => {
          ...
          setTimeout(() => {
            if (this.state.offerRadio == 1) {
              payload = {
                ...
                leaseStartDate: this.state.leaseEndDate,
                leaseType: this.state.leaseType,
                tenantName: this.state.firstName,
              };
            } else {
              payload = {
                appReqId: this.state.tenantId,
                leaseEndDate: values.leaseEnd,
                leaseStartDate: this.state.leaseEndDate,
            ...
            <Form onSubmit={props.handleSubmit}>
              ...
              <Card>
                <CardBody>
                  ...
                  <label htmlFor="genderType">Renewal Type</label>
                  <FormGroup className="has-icon-left position-relative">
                    <div className="d-inline-block mr-1">
                      <Radio
                        ...
                        color="primary"
                        name="offerRadio"
                        checked={offerRadio == 1}
                        onChange={this.renewalRadioOffer}
                      />
                    </div>
                    <div className="d-inline-block mr-1">
                      <Radio
                        ...
                        color="warning"
                        name="offerRadio"
                        checked={offerRadio == 2}
                        onChange={this.renewalRadioManual}
                      />
                    </div>
                  </FormGroup>
